...
public class AStar {
	...
	public static void main(String[] args) {
		...
		//Depth-first generated maze
		Node[][] grid = gridFromMaze(DFSMaze.makeMaze(8, 8));
		
		//Random grid of walls and spaces
		//Node[][] grid = createGrid(8, 8);
		
		AStar.getPath(grid, 1, 1, grid[0].length - 2, grid.length - 2);
		
	}
	
	public static void getPath(Node[][] grid, int startX, int startY, int endX, int endY) {
		...
		while(!openList.isEmpty()) {
			...
			if(currentNode == endNode) {
				pathFound = true;
				printGrid(grid, getPath(endNode));
			}
